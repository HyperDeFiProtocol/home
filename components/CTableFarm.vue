      ...
      <div class='shadow overflow-hidden border-b border-gray-700'>
        <table class='table-history'>
          ...
          <tr v-for='tx in transactions'>
            ...
            <td v-if='tx.sender === $store.state.bsc.globalAccounts.tax' class='text-emerald-400'>
              {{ $t('txTable.harvest') }}
            </td>
            <td v-else>
              {{ $t('txTable.accumulate') }}
            </td>
            <td v-if='tx.sender === $store.state.bsc.globalAccounts.tax' class='font-mono text-emerald-400'>
              <a target='_blank' :href='hpLink.exploreTx(tx.txHash)'>
